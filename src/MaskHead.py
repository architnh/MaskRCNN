...
class MaskHead(torch.nn.Module):
    ...
    def preprocess_ground_truth_creation(self, proposals, class_logits, box_regression, gt_labels,bbox ,masks , IOU_thresh=0.5, keep_num_preNMS=1000, keep_num_postNMS=100):
        ...
        #
        # Input:
        #       proposals: len(bz)(per_image_proposals,4)
        ...
        for i, each_proposal in enumerate(proposals):
            ...
            if len(non_bg_label_idx) != 0: 
                ...
                max_iou_val = torch.max(IOU(sorted_boxes_pre_nms, bbox[i]), dim=1)[0]
                max_iou_above_thres = torch.where(max_iou_val > 0.5)

                sorted_boxes_pre_nms = sorted_boxes_pre_nms[max_iou_above_thres]
                sorted_clases_pre_nms = sorted_clases_pre_nms[max_iou_above_thres]
                sorted_scores_pre_nms = sorted_scores_pre_nms[max_iou_above_thres]
                ...
                clases_post_nms, scores_post_nms, boxes_post_nms = self.nms_preprocess_gt(clases_pre_nms, boxes_pre_nms, scores_pre_nms, IOU_thres=IOU_thresh, keep_num_postNMS=keep_num_postNMS)

                gt_mask_one = torch.zeros(clases_post_nms.shape[0],3,self.image_size[0], self.image_size[1])
                for j in range(clases_post_nms.shape[0]):
                    gt_mask_one[j,:,boxes_post_nms[j,1]:boxes_post_nms[j,3],boxes_post_nms[j,0]:boxes_post_nms[j,2]] = 1
                    msk_target = torch.stack([gt_mask_one[j,k]*masks[i][k] for k in gt_labels[i]])
                    msk_target = F.interpolate(msk_target.unsqueeze(0), size=(2*self.P,2*self.P),mode='nearest')
                    gt_mask_one[j] = msk_target.squeeze(0)
    ...
    def postprocess_mask(self, masks_outputs, labels):
        ...
        # Input:
        #       masks_outputs: len(bz) (total_boxes,C,2*P,2*P)
        #       labels: list:len(bz){(post_NMS_boxes_per_image)}  (top category of each regressed box)
        # Output:
        #       projected_masks: list:len(bz){(post_NMS_boxes_per_image,image_size[0],image_size[1]
        projected_masks = []
        for i in range(len(masks_outputs)):
            one_projected_mask = [masks_outputs[:,k] for k in labels[i]]  
            one_projected_mask = F.interpolate(one_projected_mask.unsqueeze(0), size=self.image_size, mode="bilinear")
